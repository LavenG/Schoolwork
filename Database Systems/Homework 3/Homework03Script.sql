...
AS
	SELECT EIN, NAME
	FROM COMPANY
	WHERE EIN NOT IN (SELECT EIN FROM SHIPPER)
	AND EIN NOT IN (SELECT EIN FROM CUSTOMER)
	AND EIN NOT IN (SELECT EIN FROM VENDOR);
...
AS
	SELECT co.NAME AS CUSTOMER_NAME,
		     c.CREDIT AS CREDIT_RATING,
		     p.PRODUCTNAME AS PRODUCT_NAME
	FROM CUSTOMER c, COMPANY co, PRODUCT p, ORDERS o
	WHERE c.EIN = co.EIN AND
		    c.EIN = o.CUSTOMEREIN AND
...
AS
	SELECT co.NAME AS SHIPPER_NAME,
		     s.EIN AS SHIPPER_EIN,
		     s.PRICEPERLB AS PRICE_LB
	FROM COMPANY co, SHIPPER s, WAREHOUSE w
	WHERE s.EIN = co.EIN
	AND s.EIN = w.SHIPPEREIN
	GROUP BY SHIPPER_EIN
	HAVING COUNT(s.EIN) > 2
	ORDER BY SHIPPER_NAME DESC;
...
AS
	SELECT DISTINCT p1.PRODUCTNAME AS PRODUCT_1,
									p2.PRODUCTNAME AS PRODUCT_2
	FROM PRODUCT p1, PRODUCT p2, PRODUCTUSAGE pu1, PRODUCTUSAGE pu2
	WHERE p1.PRODUCTNAME = pu1.PRODUCTNAME
	AND p1.PRODUCTID = pu1.PRODUCTID
	AND p2.PRODUCTNAME = pu2.PRODUCTNAME
	AND p2.PRODUCTID = pu2.PRODUCTID
	AND pu1.PRODUCTUSAGEVALUE > pu2.PRODUCTUSAGEVALUE
...
AS
	SELECT c.EIN AS CUSTOMER_EIN,
		     co.NAME AS CUSTOMER_NAME
	FROM CUSTOMER c, COMPANY co, ORDERS o,
		(SELECT PRODUCTNAME, PRODUCTID
		 FROM CUSTOMER, COMPANY, ORDERS
		 WHERE COMPANY.NAME = 'IBM'
		 AND CUSTOMER.EIN = COMPANY.EIN
		 AND CUSTOMER.EIN = ORDERS.CUSTOMEREIN) AS IBMPRODUCTS
	WHERE c.EIN = co.EIN
	AND c.EIN = o.CUSTOMEREIN
	AND co.NAME != 'IBM'
	AND o.PRODUCTID = IBMPRODUCTS.PRODUCTID
	AND o.PRODUCTNAME = IBMPRODUCTS.PRODUCTNAME
...
AS
	SELECT p.PRODUCTNAME AS PRODUCT_NAME,
		     p.PRICE, i.ITEMSIZE
	FROM PRODUCT p, ITEM i, CONTAINS c
	WHERE p.PRODUCTID = c.PRODUCTID
	AND p.PRODUCTNAME = c.PRODUCTNAME
...
AS
	SELECT co.NAME AS CUSTOMER_NAME,
		     SUM(o.QUANTITY * s.PRICEPERLB * p.WEIGHTINLB) AS SHIPMENT_PRICE
	FROM COMPANY co, ORDERS o, PRODUCT p, SHIPPER s
	WHERE co.EIN = o.CUSTOMEREIN
	AND s.EIN = o.SHIPPEREIN
